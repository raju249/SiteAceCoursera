...
var fs = require('fs');                                                // 1
                                                                       //
// Setup env                                                           //
process.env.ROOT_URL = process.env.OPENSHIFT_APP_DNS || "localhost";   // 4
process.env.MONGO_URL = process.env.OPENSHIFT_MONGODB_DB_URL + process.env.OPENSHIFT_APP_NAME || "mongodb://localhost:27017/meteor";
process.env.PORT = process.env.OPENSHIFT_NODEJS_PORT || 8000;          // 6
process.env.BIND_IP = process.env.OPENSHIFT_NODEJS_IP || '127.0.0.1';  // 7
                                                                       //
// Show connection details on startup                                  //
console.log("MONGO_URL IS: " + process.env.MONGO_URL);                 // 10
console.log("ROOT_URL IS: " + process.env.ROOT_URL);                   // 11
console.log("PORT: " + process.env.PORT);                              // 12
console.log("BIND_IP: " + process.env.BIND_IP);                        // 13
                                                                       //
fs.stat('main.js', function (err, stat) {                              // 15
  ...
  if (!err) {                                                          // 18
    ...
    http.createServer(function (req, res) {                            // 26
      res.writeHead(200, { 'Content-Type': 'text/html' });             // 27
      res.end(dev_instructions);                                       // 28
...
//# sourceMappingURL=meteorshim.js.map
